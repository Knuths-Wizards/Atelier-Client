    ...
    >
      ...
      >
        ...
        >
          <ScrollMenu
            LeftArrow={
              <LeftArrow setCurrentStyle={setCurrentStyle}></LeftArrow>
            }
        ...
        <button
          ...
          style={{ position: "absolute", top: "0.5%", left: "82.5%" }}
          onClick={showModal}
        >
          <svg
        ...
        <dialog ref={modalRef} className="modal">
          <form method="dialog" className="modal-box">
            <ModalContent
              ogFeatures={ogProduct.features}
              features={features}
              title={title}
              ogTitle={ogProduct.name}
            />
          </form>
      ...
      <div
        className="card-actions text-center"
      ...
      <div
        ...
        style={{ position: "absolute", top: "53.2%", padding: "10px" }}
      >
        <div className="flex flex-col items-center mt-4 flex-grow">
          ...
          <div
            width="135px"
            height="56px"
            style={{ marginTop: "0px", width: "135px", height: "56px" }}
          >
            <h3
              className="card-title text-center justify-center"
              style={{ cursor: "pointer" }}
              onClick={() => setNewProduct(itemId)}
            >
              {title}
            </h3>
          </div>
    ...
    <table className="table table-zebra bg-info">
      ...
      <tbody>
        {mergedFeatures.map((feature, index) => {
          let featuresValue =
            features.find((item) => item.feature === feature)?.value || "X";
          let ogFeaturesValue =
            ogFeatures.find((item) => item.feature === feature)?.value || "X";
