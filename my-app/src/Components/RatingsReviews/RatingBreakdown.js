...
const RatingBreakdown = (props) => {
  ...
  for (let key in characteristics) {
    ...
    charElements.push(
      <div key={key} className="grid grid-cols-3 w-12/12">
        <span className="col-start-1 place-content-start p-1">{key}:</span>
        <span className="col-start-2 col-span-2 place-content-start p-1">
          {charLegend[key][index]}
        </span>
      </div>,
    );
  }
  ...
  for (let i = 5; i > 0; i--) {
    starRatings.push(
      <div className="flex items-center" key={`breakdown-${i}`}>
        ...
        <span className="p-1">({ratings[i]} reviews)</span>
      </div>,
    );
  }
