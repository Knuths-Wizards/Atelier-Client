...
describe("Question", () => {
  ...
  it('renders more answers when the "Load More Answers" button is clicked', () => {
    ...
    expect(updatedAnswerComponents.length).toBeGreaterThan(
      initialAnswerComponents.length,
    );
  });
  ...
  it('increments the helpfulness count by 1 when the "Helpful" button is clicked', () => {
    render(<Question question={question} />);
    const helpfulButton = screen.getByText(
      `Yes (${question.question_helpfulness})`,
    );
    fireEvent.click(helpfulButton);
    expect(helpfulButton.textContent).toBe(
